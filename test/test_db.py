...
def pytest_generate_tests(metafunc):
    """ Generate tests """
    if metafunc.cls:
        metafunc.parametrize('db', metafunc.cls.databases,
                             ids=['sqlite', 'mysql'])
...
class TestDB:
    """
    Run parametrized tests for fifferent DB types
    """
    ...
    databases = [
        "test.db",
        pytest.mark.mysqltest(mysql_db),
    ]

    def clean(self):
        session = jip.db.create_session()
        map(lambda x: session.execute('drop table %s;' % x),
            [x[0] for x in list(session.execute('show tables;'))])
        jip.db.save([])


    def test_command_change_save(self, db, tmpdir):
    ...
    def test_updating_state(self, db, tmpdir):
        if not db.startswith('mysql'):
            db = os.path.join(str(tmpdir), db)
        jip.db.init(db)
        j = jip.db.Job()
    ...
    def test_update_archived_single(self, db, tmpdir):
        if not db.startswith('mysql'):
            db = os.path.join(str(tmpdir), db)
        jip.db.init(db)
        j = jip.db.Job()
    ...
    def test_update_archived_multiple(self, db, tmpdir):
        if not db.startswith('mysql'):
            db = os.path.join(str(tmpdir), db)
        jip.db.init(db)
        j1 = jip.db.Job()
    ...
    def test_update_state_of_non_existing_job(self, db, tmpdir):
        if not db.startswith('mysql'):
            db = os.path.join(str(tmpdir), db)
        jip.db.init(db)
        j = jip.db.Job()
    ...
    def test_delete_single_job(self, db, tmpdir):
        if not db.startswith('mysql'):
            db = os.path.join(str(tmpdir), db)
        jip.db.init(db)
        j = jip.db.Job()
    ...
    def test_get_state(self, db, tmpdir):
        if not db.startswith('mysql'):
            db = os.path.join(str(tmpdir), db)
        jip.db.init(db)
        j = jip.db.Job()
    ...
    def test_delete_job_with_parent_job(self, db, tmpdir):
        if not db.startswith('mysql'):
            db = os.path.join(str(tmpdir), db)
        jip.db.init(db)
        parent = jip.db.Job()
    ...
    def test_delete_unknown_job(self, db, tmpdir):
        if not db.startswith('mysql'):
            db = os.path.join(str(tmpdir), db)
        jip.db.init(db)
        job = jip.db.Job()
    ...
    def test_cascaded_save(self, db, tmpdir):
        if not db.startswith('mysql'):
            db = os.path.join(str(tmpdir), db)
        jip.db.init(db)
        parent = jip.db.Job()
    ...
    def test_ouput_file_update_on_insert(self, db, tmpdir):
        if not db.startswith('mysql'):
            db = os.path.join(str(tmpdir), db)
        jip.db.init(db)
        p = jip.Pipeline()
    ...
    def test_input_file_update_on_insert(self, db, tmpdir):
        if not db.startswith('mysql'):
            db = os.path.join(str(tmpdir), db)
        jip.db.init(db)
        p = jip.Pipeline()
    ...
    def test_input_file_delete_on_job_delete(self, db, tmpdir):
        if not db.startswith('mysql'):
            db = os.path.join(str(tmpdir), db)
        jip.db.init(db)
        p = jip.Pipeline()
    ...
    def test_output_file_delete_on_job_delete(self, db, tmpdir):
        if not db.startswith('mysql'):
            db = os.path.join(str(tmpdir), db)
        jip.db.init(db)
        p = jip.Pipeline()
    ...
    def test_output_file_query(self, db, tmpdir):
        if not db.startswith('mysql'):
            db = os.path.join(str(tmpdir), db)
        jip.db.init(db)
        p = jip.Pipeline()
    ...
    def test_file_query_and(self, db, tmpdir):
        if not db.startswith('mysql'):
            db = os.path.join(str(tmpdir), db)
        jip.db.init(db)
        p = jip.Pipeline()
    ...
    def test_output_file_query_multiple_files(self, db, tmpdir):
        if not db.startswith('mysql'):
            db = os.path.join(str(tmpdir), db)
        jip.db.init(db)
        p = jip.Pipeline()
    ...
    def test_input_file_query(self, db, tmpdir):
        if not db.startswith('mysql'):
            db = os.path.join(str(tmpdir), db)
        jip.db.init(db)
        p = jip.Pipeline()
    ...
    def test_iput_file_query_multiple_files(self, db, tmpdir):
        if not db.startswith('mysql'):
            db = os.path.join(str(tmpdir), db)
        jip.db.init(db)
        p = jip.Pipeline()
    ...
    def test_no_duplicated_jobs(self, db, tmpdir):
        if not db.startswith('mysql'):
            db = os.path.join(str(tmpdir), db)
        jip.db.init(db)
        p = jip.Pipeline()
    ...
    def test_no_duplicated_jobs_after_file_query_direct(self, db, tmpdir):
        if not db.startswith('mysql'):
            db = os.path.join(str(tmpdir), db)
        jip.db.init(db)
        p = jip.Pipeline()
